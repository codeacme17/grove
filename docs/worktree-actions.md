...
Each available, non-bare worktree card provides a Pull icon in its top-right corner. Click the path text to switch branches; Show Diff sits at the right of the path row. These extend the original read-only browser. Copy Path and Reveal in Finder remain in the card menu.
...
- The refresh icon reloads the file list. Closing the detail panel cancels its unfinished read. Collapsing Show Diff hides the file list while keeping the selected file open on the right. Switching projects closes the detail panel. An expanded panel reloads after a project refresh or worktree operation.
